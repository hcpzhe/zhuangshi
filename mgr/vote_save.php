...
if($action == 'add')
{
	...
	$sql = "INSERT INTO `$tbname` (title, content, starttime, endtime, isguest, isview, intval, isradio, orderid, posttime, checkinfo) VALUES ('$title', '$content', '$starttime', '$endtime', '$isguest', '$isview', '$intval', '$isradio', '$orderid', '$posttime', '$checkinfo')";
...
if($action == 'update')
{
	...
	$sql = "UPDATE `$tbname` SET title='$title', content='$content', starttime='$starttime', endtime='$endtime', isguest='$isguest', isview='$isview', intval='$intval', isradio='$isradio', orderid='$orderid', posttime='$posttime', checkinfo='$checkinfo' WHERE id=$id";
	if($dosql->ExecNoneQuery($sql))
	{
		header("location:$gourl");
		exit();
