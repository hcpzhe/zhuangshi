...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
	...
	<?php
	$sql = "SELECT * FROM `#@__admin` WHERE id <> 2";

	if($cutover_url == 'small') $sql .= " AND levelname <> 0";
	...
	<tr align="center" class="mgr_tr">
		...
		<td class="action"><span>[<a href="admin_save.php?id=<?php echo $row['id']; ?>&action=check&checkadmin=<?php echo $row['checkadmin']; ?>" title="点击进行审核与未审操作"><?php echo $checkadmin; ?></a>]</span><span>[<a href="admin_update.php?id=<?php echo $row['id']; ?>">修改</a>]</span><span>[<a href="admin_save.php?action=del&id=<?php echo $row['id']; ?>" onclick="return ConfDel(0);">删除</a>]</span></td>
	</tr>
	<?php
	}
	?>
