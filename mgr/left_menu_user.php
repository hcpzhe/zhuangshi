...
<div id="scrollmenu">
	...
	<div class="viewport">
		<div class="overview">
			...
			?>
			<div class="menubox">
				<div class="title" onclick="DisplayMenu('leftmenu1');" title="点击进行显示隐藏操作">网站系统管理</div>
				<div id="leftmenu1"><a href="admin.php" target="main">管理员管理</a> <a href="web_config.php" target="main">网站信息配置</a> <a href="upload_filemgr.php" target="main">上传文件管理</a></div>
			</div>
			<div class="hr_5"></div>
			<?php
			}
			
			$dosql->Execute("SELECT * FROM `#@__infoclass` WHERE parentid=0 AND adminmenu='true' ORDER BY orderid ASC");
			$i = 10;
			while($row = $dosql->GetArray())
			{
			?>
			<div class="menubox">
				<div class="title" onclick="DisplayMenu('leftmenu<?php echo $i; ?>');" title="点击进行显示隐藏操作"><?php echo $row['classname']; ?></div>
				...
				$dosql->Execute("SELECT * FROM `#@__infoclass` WHERE 1 AND (id=".$row['id']." OR parentstr LIKE '0,".$row['id'].",%') ORDER BY orderid ASC",$i);
