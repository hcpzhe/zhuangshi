...
<body>
<div class="mgr_header"> <span class="title">商品属性管理</span> <span class="reload"><a href="javascript:location.reload();">刷新</a></span> </div>
<form name="form" id="form" method="post" action="goodsattr_save.php?action=save">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
		...
		<?php
		$dosql->Execute("SELECT * FROM `#@__goodsattr` ORDER BY orderid ASC");
		...
		<tr align="center" class="mgr_tr">
			...
			<td align="left"><input name="classname[]" type="text" id="classname[]" class="input_gray" value="<?php echo $row['classname']; ?>" /></td>
			<td><a href="goodsattr_save.php?id=<?php echo $row['id']; ?>&orderid=<?php echo $row['orderid']; ?>&action=up"><img src="templets/images/up.gif" title="向上移动" /></a>
				<input name="orderid[]" type="text" id="orderid[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
				<a href="goodsattr_save.php?id=<?php echo $row['id']; ?>&orderid=<?php echo $row['orderid']; ?>&action=down"><img src="templets/images/down.gif" title="向下移动" /></a></td>
			<td class="action"><span>[<a href="goodsattr_save.php?id=<?php echo $row['id']; ?>&action=check&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行显示与隐藏操作"><?php echo $checkinfo; ?></a>]</span><span>[<a href="goodsattr_save.php?action=del2&id=<?php echo $row['id'] ?>" onclick="return ConfDel(0);">删除</a>]</span></td>
		</tr>
		<?php
		}
		?>
