...
<form name="form" id="form" method="post" action="">
	...
	function Show($id=0, $i=0)
	{
		...
		while($row = $dosql->GetArray($id))
		{
			...
			if($row['parentid'] == '0')
			{
				$classname .= '<span class="disimg" id="rowid_'.$row['id'].'" onclick="DisplayRows('.$row['id'].');"></span>';
			}
			else
			{
				$classname .= '<span class="sub_type"></span>';
			}

			$classname .= '<a href="'.$addurl.'" title="点击添加内容">'.$row['classname'].'</a>';
	...
	<div rel="rowpid_<?php echo $topid ; ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
			<tr align="center" class="mgr_tr">
				...
				<td width="15%"><a href="infoclass_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/up.gif" title="向上移动" /></a>
					<input type="text" name="orderid[]" id="orderid[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
					<a href="infoclass_save.php?action=down&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/down.gif" title="向下移动" /></a></td>
				<td width="30%" class="action"><span><a href="infoclass_add.php?infotype=<?php echo $row['infotype']; ?>&id=<?php echo $row['id']; ?>">添加子栏目</a></span> | <span><a href="infoclass_save.php?action=check&id=<?php echo $row['id']; ?>&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行显示与隐藏操作"><?php echo $checkinfo; ?></a></span> | <span><a href="infoclass_update.php?id=<?php echo $row['id']; ?>">修改</a></span> | <span><a href="infoclass_save.php?action=del&id=<?php echo $row['id'] ?>" onclick="return ConfDel(1);">删除</a></span>
				</td>
			</tr>
		</table>
	</div>
...
<div class="mgr_divb">
	<div class="selall"><span>选择：</span> <a href="javascript:CheckAll(true);">全部</a> - <a href="javascript:CheckAll(false);">无</a> - <a href="javascript:DelAll('infoclass_save.php');" onclick="return ConfDelAll(1);">删除</a>　<span>操作：</span><a href="javascript:UpOrderID('infoclass_save.php');">排序</a> - <a href="javascript:ShowAllRows();">展开</a> - <a href="javascript:HideAllRows();">隐藏</a></div>
	<span class="mgr_btn"><a href="infoclass_add.php">添加网站栏目</a></span> </div>
<div class="page_area">
	<div class="page_info">共有<span><?php echo $dosql->GetTableRow("#@__infoclass"); ?></span>条记录</div>
</div>
</body>
</html>