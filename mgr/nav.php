...
<form name="form" id="form" method="post" action="">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
		<tr align="center" class="thead">
			...
			<td width="35%" class="title2">导航名称</td>
			<td width="15%">跳转链接</td>
			<td width="15%">排序</td>
			<td width="30%">操作</td>
		</tr>
	...
	{
		global $dosql;
		$dosql->Execute("SELECT * FROM `#@__nav` WHERE parentid=$id ORDER BY orderid ASC", $id);
		$i++;
	...
	<div rel="rowpid_<?php echo $topid ; ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
			<tr align="center" class="mgr_tr">
				...
				echo $classname;
				?></td>
				<td width="15%" class="number"><?php echo $row['linkurl']; ?>
					<input type="hidden"  name="id[]"id="id[]" value="<?php echo $row['id']; ?>" /></td>
				<td width="15%"><a href="nav_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/up.gif" title="向上移动" /></a>
					<input type="text" name="orderid[]" id="orderid[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
					<a href="nav_save.php?action=down&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/down.gif" title="向下移动" /></a></td>
...
<div class="page_area">
	<div class="page_info">共有<span><?php echo $dosql->GetTableRow("#@__nav"); ?></span>条记录</div>
</div>
</body>
</html>