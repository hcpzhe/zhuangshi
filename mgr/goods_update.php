...
<script type="text/javascript" src="templets/js/getjcrop.js"></script>
...
function GetAttr(tid)
{
	$.ajax({
		...
		dataType:'html',
		beforeSend:function(){
			$("#getattr").html('<div class="loading" style="width:140px;margin:0 auto;"><img src="templets/images/loading.gif">自定义属性读取中...</div>');
		},
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="form_table">
		<tr>
			<td width="25%" height="35" align="right">商品分类：</td>
			<td width="75%">
			<select name="classid" id="classid" onchange="GetAttr(this.value)">
				<option value="-1">请选择所属分类</option>
				<?php ShowType('#@__goodstype','#@__goods','classid'); ?>
			</select>
		...
		<tr>
			...
			<td><select name="brandid" id="brandid">
					...
					<?php ShowType('#@__goodsbrand','#@__goods','brandid'); ?>
				</select></td>
		</tr>
		<tr>
		...
		<tr class="nb">
			...
			<?php
			$attr = array_combine(explode(',',$row['attrid']), explode(',',$row['attrvalue']));
			$r    = $dosql->GetOne('SELECT attrstr FROM `#@__goodstype` WHERE id='.$row['classid']);
			if($r['attrstr'] != '-1')
			{
			...
			?>
			<tr>
				<td width="25%" height="35" align="right"><?php echo $row2['classname']; ?>：</td>
					<td><input type="text" name="attrvalue[]" id="attrvalue[]" class="input_short" value="<?php echo $attr[$row2['id']] ?>" />
						<input type="hidden" name="attrid[]" id="attrid[]" value="<?php echo $row2['id']; ?>">
						<?php if($i == 0){echo '<span class="cnote">不同属性值用 <span class="red">|</span> 隔开，例如：黑色|白色|粉色 或 12码|14码 等</span>';} ?></td>
			</tr>
			<?php
					$i++;
				}
		...
		<tr>
			...
			<td><input name="marketprice" type="text" id="marketprice" class="class_input" style="width:50px;" value="<?php echo $row['marketprice']; ?>" />
				市场价　
				<input name="salesprice" type="text" id="salesprice" class="class_input" style="width:50px;" value="<?php echo $row['salesprice']; ?>" />
				销售价　
				<input name="memberprice" type="text" id="memberprice" class="class_input" style="width:50px;" value="<?php echo $row['memberprice']; ?>" />
				折扣价</td>
		</tr>
		<tr>
			<td height="35" align="right">货　号：</td>
			<td><input type="text" name="goodsid" id="goodsid" class="input_short" value="<?php echo $row['goodsid']; ?>" /></td>
		</tr>
		<tr>
			<td height="35" align="right">重　量：</td>
		...
		<tr>
			<td height="35" align="right">积分点数：</td>
