...
<body>
<div class="gray_header"> <span class="title">添加商品分类</span> <span class="reload"><a href="javascript:location.reload();">刷新</a></span> </div>
<form name="form" id="form" method="post" action="goodstype_save.php" onsubmit="return cfm_btype();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="form_table">
		<tr>
			<td width="25%" height="35" align="right">所属商品类别：</td>
			<td width="75%">
			<select name="parentid" id="parentid">
				<option value="0">默认分类</option>
				<?php ShowType('#@__goodstype','#@__goodstype','id'); ?>
			</select>
		...
		<tr>
			...
			<?php
			$dosql->Execute("SELECT * FROM `#@__goodsattr` WHERE checkinfo=true ORDER BY orderid");
			$i = 1;
			...
			if($dosql->GetTotalRow() > 0)
			{
				...
				{
					echo '<span class="goodstypeattr"><input type="checkbox" name="attrids[]" id="attrids[]" value="'.$row['id'].'" />'.$row['classname'].'</span>';
					if($i % 5 == 0) echo '<br />';
					$i++;
				}
			}
		...
		<tr>
			<td height="35" align="right">跳转地址：</td>
			<td><input type="text" name="linkurl" id="linkurl" class="class_input" /></td>
		</tr>
		<tr>
			<td height="35" align="right">排列顺序：</td>
			<td><input type="text" name="orderid" id="orderid" value="<?php echo GetOrderID('#@__goodstype'); ?>" class="input_shortb" /></td>
		</tr>
		<tr class="nb">
