...
<form name="form" id="form" method="post" action="?action=export">
	...
	<div class="mgr_divb"> <span class="selall"><span>选择：</span><a href="javascript:CheckAll(form,true);">全部</a> - <a href="javascript:CheckAll(form,false);">无</a> - <a href="#" onclick="Repair('?action=export&dopost=repair')">修复</a> - <a href="#" onclick="Repair('?action=export&dopost=optimize')">优化</a> </span> <span class="mgr_btn"><a href="#" onclick="Repair('?action=export&dopost=backup')">开始备份数据</a></span> <span class="db_backopt">指定格式：
		<input type="radio" name="datatype" value="4.0" <?php if($mysql_version<4.1) echo "checked='checked'";?> />
		3.x/4.0.x&nbsp;
		<input type="radio" name="datatype" value="4.1" <?php if($mysql_version>=4.1) echo "checked='checked'";?> />
		4.1.x/5.x&nbsp;&nbsp;
		表结构：
		<input type="checkbox" name="isstruct" value="1" checked="checked" />
		&nbsp;&nbsp;
		分卷大小：
		<input type="text" name="fsize" value="2048" size="5" />
		KB&nbsp; </span> </div>
</form>
<div class="page_area">
