...
<form name="form" id="form" method="post">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
		...
		<?php
		$row = $dosql->GetOne("SELECT * FROM `#@__infoclass` WHERE id=$tid");
		...
		<tr align="center" class="mgr_tr">
			...
			<td class="action"><span><a href="infoclass_add.php?infotype=<?php echo $row['infotype']; ?>&id=<?php echo $row['id']; ?>">添加子栏目</a></span> | <span><a href="infoclass_save.php?id=<?php echo $row['id']; ?>&action=check&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行显示与隐藏操作"><?php echo $checkinfo; ?></a></span> | <span><a href="infoclass_update.php?v=true&id=<?php echo $row['id']; ?>">修改</a></span> | <span>删除</span></td>
		</tr>
	</table>
	<?php
	...
	<div rel="rowpid_<?php echo $parentstr_id; ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
			<tr align="center" class="mgr_tr">
				...
				<td width="15%" align="center"><a href="infoclass_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/up.gif" title="向上移动" /></a>
				...
				<td width="30%" class="action"><span>
					...
					<?php }else{echo '添加子栏目';} ?>
					</span> | <span><a href="infoclass_save.php?id=<?php echo $row['id']; ?>&action=check&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行显示与隐藏操作"><?php echo $checkinfo; ?></a></span> | <span><a href="infoclass_update.php?id=<?php echo $row['id']; ?>">修改</a></span> | <span><a href="infoclass_save.php?action=del&id=<?php echo $row['id'] ?>" onclick="return ConfDel(1)">删除</a></span></td>
			</tr>
		</table>
	</div>
	<?php
	...
	Show($tid, 3, $menulevel);
	?>
	<div class="mgr_divb">
		<div class="selall"><span>选择：</span> <a href="javascript:CheckAll(true);">全部</a> - <a href="javascript:CheckAll(false);">无</a> - <a href="javascript:DelAll('infoclass_save.php');" onclick="return ConfDelAll(1);">删除</a>　<span>操作：</span><a href="javascript:UpOrderID('infoclass_save.php');">更新排序</a></div>
	</div>
