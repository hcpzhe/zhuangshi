...
<form name="form" id="form" method="post" action="diymenu_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
		...
		<?php
		$dosql->Execute("SELECT * FROM `#@__diymenu` WHERE parentid=0 ORDER BY orderid ASC");
		...
		<tr align="center" class="mgr_tr">
			...
			<td>&nbsp;</td>
			<td><a href="diymenu_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/up.gif" title="向上移动" /></a>
				<input type="text" name="orderid[]" id="orderid[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
				<a href="diymenu_save.php?action=down&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/down.gif" title="向下移动" /></a></td>
		...
		?>
		<tr align="center" class="mgr_tr">
			<td height="32"><input type="checkbox" name="checkid[]" id="checkid[]" value="<?php echo $row2['id'] ?>" /></td>
			...
			<td><a href="diymenu_save.php?id=<?php echo $row2['id']; ?>&parentid=<?php echo $row2['parentid']; ?>&orderid=<?php echo $row2['orderid']; ?>&action=up"><img src="templets/images/up.gif" title="向上移动" /></a>
				<input type="text" name="orderid[]" id="orderid[]" class="input_gray_short" value="<?php echo $row2['orderid']; ?>" />
				<a href="diymenu_save.php?id=<?php echo $row2['id']; ?>&parentid=<?php echo $row2['parentid']; ?>&orderid=<?php echo $row2['orderid']; ?>&action=down"><img src="templets/images/down.gif" title="向下移动" /></a></td>
...
<div class="page_area">
	<div class="page_info">共有<span><?php echo $dosql->GetTableRow("#@__diymenu"); ?></span>条记录</div>
</div>
</body>
</html>