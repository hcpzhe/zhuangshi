...
<body>
<div class="mgr_header"> <span class="title">链接分类管理</span> <a href="weblink.php" class="postype2">[链接管理]</a> <span class="reload"><a href="javascript:location.reload();">刷新</a></span> </div>
<form name="form" id="form" method="post" action="weblinktype_save.php">
	...
	{
		global $dosql;
		$dosql->Execute("SELECT * FROM `#@__weblinktype` WHERE parentid=$id ORDER BY orderid ASC", $id);
		$i++;
	...
	<div rel="rowpid_<?php echo $parentstr_id; ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
			<tr align="center" class="mgr_tr">
				...
				<td width="15%"><a href="weblinktype_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/up.gif" title="向上移动" /></a>
					<input type="text" name="orderid[]" id="orderid[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
					<a href="weblinktype_save.php?action=down&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/down.gif" title="向下移动" /></a>
...
<div class="page_area">
	<div class="page_info">共有<span><?php echo $dosql->GetTableRow("#@__weblinktype"); ?></span>条记录</div>
</div>
</body>
</html>
