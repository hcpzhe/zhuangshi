...
<form name="form" id="form" method="post" action="diymenu_save.php" onsubmit="return cfm_diymenu();">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="mgraction">
		...
		<tr align="center" class="mgr_tr">
			...
			<td><a href="diymenu_save.php?id=<?php echo $row['id']; ?>&orderid=<?php echo $row['orderid']; ?>&action=up"><img src="templets/images/up.gif" title="向上移动" /></a>
				<input type="text" name="orderidupdate[]" id="orderidupdate[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
				<a href="diymenu_save.php?id=<?php echo $row['id']; ?>&orderid=<?php echo $row['orderid']; ?>&action=down"><img src="templets/images/down.gif" title="向下移动" /></a></td>
			<td class="action"><span>[<a href="diymenu_save.php?action=del2&id=<?php echo $row['id'] ?>" onclick="return ConfDel(0);">删除</a>]</span></td>
		</tr>
		<?php
		}
		?>
		...
		<tr align="center" class="mgr_tr">
			<td height="32"><a href="javascript:;" onclick="AddDmTr()"><img src="templets/images/add_row.gif" title="新增一行" /></a></td>
	...
	<div class="mgr_divb">
		...
		<span class="mgr_btn_short"><a href="#" onclick="form.submit();">更新全部</a><a href="diymenu.php" style="margin-right:5px;">返回管理</a></span> </div>
