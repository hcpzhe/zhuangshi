	...
	<div rel="rowpid_<?php echo $topid; ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
			<tr align="center" class="mgr_tr">
				...
				<td width="15%"><a href="goodstype_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/up.gif" title="向上移动" /></a>
					<input type="text" name="orderid[]" id="orderid[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
					<a href="goodstype_save.php?action=down&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/down.gif" title="向下移动" /></a></td>
				<td width="30%" class="action"><span>[<a href="goodstype_add.php?id=<?php echo $row['id']; ?>">添加子类别</a></span><span><a href="goodstype_save.php?id=<?php echo $row['id']; ?>&action=check&checkinfo=<?php echo $row['checkinfo']; ?>" title="点击进行显示与隐藏操作"><?php echo $checkinfo; ?></a></span><span><a href="goodstype_update.php?id=<?php echo $row['id']; ?>">修改</a></span><span><a href="goodstype_save.php?action=del&id=<?php echo $row['id'] ?>" onclick="return confirm_del(0);">删除</a>]</span></td>
			</tr>
		</table>
	</div>
	<?php
