...
//引入相关文件
$config_cache  = SITE_DATA.'/config.cache.php';
$watermark_inc = SITE_DATA.'/watermark/watermark.inc.php';


//设置选项卡项
$config_tab_arr = array('基本设置','附件设置','性能设置','URL静态化');
...
function RewriteURL_Str($d,$r)
{
	...
	foreach($d[1] as $k=>$v)
	{
		if(stripos(',,about,news,newsshow,product,productshow,case,caseshow,join,joinshow,message,contact,soft,softshow,vote,,',','.$v.','))
		{
		...
		{
			if(stripos(',,id,cid,page,keyword,,', ','.$v.','))
			{
				if($v == 'keyword')
				{
...
function RewriteURL()
{
	global $cfg_reurl_index, $cfg_reurl_about, $cfg_reurl_news, $cfg_reurl_newsshow,
		   ...
		   $cfg_reurl_soft, $cfg_reurl_softshow, $cfg_reurl_vote, $cfg_reurl_custom, $cfg_webpath;
	 ...
	 * 进行标记替换
	*/
	if(empty($cfg_webpath)) $webpath = '/';
	else $webpath = $cfg_webpath;
	...
	{
		ShowMsg("文件失败 rewrite.php 文件失败，可能是由于没有写入权限，因此不能更新配置！", 'web_config.php');
		exit();
	}
}
...
if($action == 'add')
{
	...
	{
		ShowMsg('变量名不能为空并必须为[a-z_]组成！', 'web_config.php');
		exit();
	}
	if($vartype=='bool' && ($varvalue!='Y' && $varvalue!='N'))
	{
		ShowMsg('布尔变量值必须为\'Y\'或\'N\'！', 'web_config.php');
		exit();
	}

	if($dosql->GetOne("SELECT `varname` FROM `#@__webconfig` WHERE varname='$varname'"))
	{
		ShowMsg('该变量名称已经存在！', 'web_config.php');
		exit();
	}
	...
	$sql = "INSERT INTO `#@__webconfig` (varname, varinfo, varvalue, vartype, vargroup, orderid) VALUES ('$varname', '$varinfo', '$varvalue', '$vartype', '$vargroup', '$orderid')";
	if(!$dosql->ExecNoneQuery($sql))
	{
		ShowMsg('新增变量失败，可能有非法字符！', 'web_config.php');
		exit();
	}
...
{
	$vars = array('cfg_markswitch','cfg_marktype','cfg_markpicurl','cfg_marktext','cfg_markcolor','cfg_marksize','cfg_markwhere');
	$str = '';
	...
	{
		ShowMsg('成功更新水印配置！', 'web_config.php');
		exit();
	}
	else
	{
		ShowMsg('保存 watermark.inc.php 文件失败，可能是由于没有写入权限，因此不能更新配置！', 'web_config.php');
		exit();
	}
}
