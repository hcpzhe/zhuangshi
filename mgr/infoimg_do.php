...
<div class="mgr_divt">
		...
		<div id="search" class="search"> <span class="s">
				...
				</span> <span class="b"><a href="javascript:;" onclick="GetSearch();"><img src="templets/images/search_btn.png" title="搜索" /></a></span></div>
</div>
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="ajaxlist">
		...
		<tr align="center" class="mgr_tr" onmouseover="this.className='mgr_tr_on'" onmouseout="this.className='mgr_tr'">
				...
				<td valign="middle"><?php echo GetMgrThumbs($row['picurl']); ?></td>
				...
				<td class="action"><span id="check<?php echo $row['id']; ?>">[<a href="javascript:;" title="点击进行审核与未审操作" onclick="CheckInfo(<?php echo $row['id']; ?>,'<?php echo $checkinfo; ?>')"><?php echo $checkinfo; ?></a>]</span><span>[<a href="infoimg_update.php?tid=<?php echo $tid; ?>&id=<?php echo $row['id']; ?>">修改</a>]</span><span>[<a href="javascript:;" onclick="ClearInfo(<?php echo $row['id']; ?>)">删除</a>]</span></td>
		</tr>
		<?php
		}	
		?>
...
<div class="mgr_divb">
		...
		{
		?>
		<span class="mgr_btn"><a href="infoimg_add.php">添加图片信息</a></span>
		<?php
		}
		?>
</div>
