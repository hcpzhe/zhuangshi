...
<form name="form" id="form" method="post" action="">
	...
	{
		global $dosql;
		$dosql->Execute("SELECT * FROM `#@__maintype` WHERE parentid=$id ORDER BY orderid ASC", $id);
		$i++;
	...
	<div rel="rowpid_<?php echo $topid; ?>">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
			<tr align="center" class="mgr_tr">
				...
				<td width="15%"><a href="maintype_save.php?action=up&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/up.gif" title="向上移动" /></a>
					<input type="text" name="orderid[]" id="orderid[]" class="input_gray_short" value="<?php echo $row['orderid']; ?>" />
					<a href="maintype_save.php?action=down&id=<?php echo $row['id']; ?>&parentid=<?php echo $row['parentid']; ?>&orderid=<?php echo $row['orderid']; ?>"><img src="templets/images/down.gif" title="向下移动" /></a></td>
...
<div class="page_area">
	<div class="page_info">共有<span><?php echo $dosql->GetTableRow("#@__maintype"); ?></span>条记录</div>
</div>
<div class="notewarn" id="notification"> <span class="close"><a href="javascript:;" id="notification_close" onclick="HideDiv('notification');"></a></span>
	<div>二级类别用于站中站模块相同的情况下使用，可以大幅调高开发效率，默认为关闭，可以在 <a href="web_config.php" class="text">网站信息配置</a> 中开启</div>
</div>
</body>
</html>