...
//初始化参数
$tbname = '#@__infolist';
$gourl  = isset($tid) ? ($gourl = "infolist.php?tid=$tid") : ($gourl = "infolist.php");
...
if($action == 'add')
{
	...
	//保存远程资源
	if($remote == 'true') $content = GetCurCont($content);
    ...
    if($autopage == 'true')
    {
        $content = AutoPage($content, $autopagesize*1024, '<hr style="page-break-after:always;" class="ke-pagebreak" />');
    }
	...
	$sql = "INSERT INTO `$tbname` (classid, parentid, parentstr, mainid, mainpid, mainpstr, title, colorval, boldval, flag, author, linkurl, keywords, description, content, picurl, picarr, orderid, hits, posttime, checkinfo {$fieldname}) VALUES ('$classid', '$parentid', '$parentstr', '$mainid', '$mainpid', '$mainpstr', '$title', '$colorval', '$boldval', '$flag', '$author', '$linkurl', '$keywords', '$description', '$content', '$picurl', '$picarr', '$orderid', '$hits', '$posttime', '$checkinfo' {$fieldvalue})";
...
else if($action == 'update')
{
	...
	$sql = "UPDATE `$tbname` SET classid='$classid', parentid='$parentid', parentstr='$parentstr', mainid='$mainid', mainpid='$mainpid', mainpstr='$mainpstr', title='$title', colorval='$colorval', boldval='$boldval', flag='$flag', author='$author', linkurl='$linkurl', keywords='$keywords', description='$description', content='$content', picurl='$picurl', picarr='$picarr', orderid='$orderid', hits='$hits', posttime='$posttime', checkinfo='$checkinfo' {$fieldstr} WHERE id=$id";
	if($dosql->ExecNoneQuery($sql))
	{
		header("location:$gourl");
		exit();
